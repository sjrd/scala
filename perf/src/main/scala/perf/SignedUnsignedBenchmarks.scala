package perf {
  ...
  @State(Scope.Benchmark) class ByteBenchmark {
    ...
    @Benchmark def fastops1 = x2.*(x3.*(x1.*(x2.+(x1)))).*(x2).+(x0.-(x2.-(x0)).*(x0).-(x3).*(x2.+(x2))).+(x3.*(x2.-(x3)).-(x2.+(x2).+(x3).*(x3).*(x3.*(x1)).-(x0)));
    @Benchmark def fastops2 = x0.+(x2.*(x0).*(x0).-(x1)).-(x1.*(x1.*(x2).+(x2).+(x0).+(x1.*(x3.*(x3.-(x0).+(x0))).-(x0)).+(x3).*(x1)));
    @Benchmark def fastops3 = x0.-(x3.*(x0.-(x2.-(x2.+(x3.+(x2.*(x3.*(x0.-(x2.*(x0.-(x3)).-(x1)).*(x1.*(x2.*(x2)).*(x0)).-(x0.+(x1)).-(x2)))))).*(x3))).-(x0)).-(x3.-(x2.-(x2).*(x2)).*(x2).+(x3)).*(x2);
    @Benchmark def fastops4 = x1.-(x0.*(x1).-(x2).+(x2.-(x2).-(x3.*(x0.*(x0.+(x0.+(x1)).*(x3).*(x0.*(x1.*(x0.-(x0))).*(x3).*(x1)))).-(x3.*(x3)).-(x0.*(x2)).+(x3))));
    @Benchmark def allops1 = x3.*(x2.%(x2.-(x0.+(x1.*(x2)).*(x2)).-(x1./(x2./(x1)).-(x1.-(x1.+(x3))).*(x2.*(x0))))).-(x0.+(x2.%(x3).*(x2).%(x1.-(x3.+(x3.+(x0.+(x1)).-(x1.%(x3)))))));
    @Benchmark def allops2 = x2.+(x0.-(x2.+(x1.*(x2)./(x1)).-(x3).+(x3.*(x1.+(x0.-(x3))./(x3.*(x0).+(x0.*(x2)./(x1))))).+(x0)))./(x3./(x2));
    @Benchmark def allops3 = x1.%(x2.+(x1).+(x0).-(x0.*(x3./(x1)./(x2))).*(x1).+(x2.%(x0)).%(x0.*(x3).+(x2)).+(x2)./(x0)).+(x3)./(x2);
    @Benchmark def allops4 = x0.-(x0)./(x2).+(x2.*(x3./(x1.+(x0./(x1).*(x1.+(x1.-(x1./(x2)./(x3.*(x2)).%(x3).%(x0.+(x2)).-(x2.%(x0)))).-(x0))))))
  }
