...
final class UByte private[scala] (private val underlying: Byte) extends AnyVal
  ...
  @inline final def max(that: UByte): UByte = math.max(this.toUInt, that.toUInt).toUByte
  @inline final def min(that: UByte): UByte = math.min(this.toUInt, that.toUInt).toUByte
